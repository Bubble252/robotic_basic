...
def task01():
    ...
    while (True):
        ...
        if (line):
            ...
            #        print(rho_err,pingyi,theta_err,value)
            # line.magnitude()返回一个表示“线性回归效果”的值，这个值越大，线性回归效果越好；
            # 如果越接近于0，说明我们的线性回归效果越接近于一个圆，效果越差
        #     if line.magnitude() > 8:
        #         #            send_data_packet(rho_err,pingyi,theta_err,value)
        #         LED(1).off()
        #     else:
        #         LED(1).on()
        #     LED(2).off()
...
#(0, 49, -30, 0, 0, 22)黑色阈值
        ...
        #        vertical_err=0
        #        vertical_err=cross_y-81
        #        vertical_output=vertical_pid.get_pid(vertical_err,1)

        #        Magnam_forward(color_ouput,vertical_output,0)
        ########################################没有检测到就后退#######################################################
        # 如果没有检测到任何色块，执行默认操作
        ...
        # 发送数据包
        #        send_color_packet(color_new, robot_err, shuiping_err)
        #print(color_new, robot_err, shuiping_err, cross_y)
...
def task03():  # 新的什么也不干函数
    ...
    #     img.draw_string(10, 10, "Hello World", color=(255, 0, 0))  # 在图像上绘制字符串
    #     #print("FPS: ", clock.fps())     # 在控制台打印帧率
